...
/*
 * thread_hw_wait_cmd() — poll flash busy status with RPC-based suspend.
 * Ported from Renesas 4.3 core into plat-rcar_gen4 for 4.10 compatibility.
 * @base_time: time measurement started
 * @timeout:   total timeout in ms
 * @wait:      RPC suspend duration in ms (passed to REE scheduler)
 * @delay:     minimum elapsed ms before issuing RPC (busy-wait until then)
 */
